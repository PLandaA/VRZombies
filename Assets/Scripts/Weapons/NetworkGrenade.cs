...
public class NetworkGrenade : NetworkBehaviour
{
    ...
    {
        var damagedZombies = new HashSet<NetworkZombie>();
        foreach (var hit in Physics.OverlapSphere(transform.position, explosionRadius))
        {
            var zombie = hit.GetComponentInParent<NetworkZombie>();
            if (zombie == null || zombie.State == NetworkZombie.ZombieState.Dead) continue;
            if (!damagedZombies.Add(zombie)) continue;

            float dist = Vector3.Distance(zombie.transform.position, transform.position);
            int dmg = Mathf.RoundToInt(Mathf.Lerp(zombieDamage, zombieDamage * 0.25f, Mathf.Clamp01(dist / explosionRadius)));
            bool killingBlow = zombie.Health <= dmg;
            zombie.RPC_TakeDamage(dmg);
            if (killingBlow) ScoreEvents.RegisterKill(false, zombie.transform.position);
        }

        if (playerDamage <= 0 || NetworkManager.instance == null) return;

        // Local physical body of the thrower
        DamagePlayerIfInRange(NetworkManager.instance.GetPlayer(Runner.LocalPlayer),
        ...
        foreach (var rig in FindObjectsByType<NetworkRig>(FindObjectsSortMode.None))
        {
            ...
            if (isLocal) continue;
            DamagePlayerIfInRange(NetworkManager.instance.GetPlayer(rig.Object.InputAuthority), rig.transform);
        }
    }

    private void DamagePlayerIfInRange(NetworkPlayer np, Transform body)
    {
        if (np == null || body == null) return;
        if (np.Object == null || !np.Object.IsValid || np.IsDead) return;
        if (Vector3.Distance(body.position, transform.position) > explosionRadius) return;
        np.RPC_TakeDamage(playerDamage);
    }
